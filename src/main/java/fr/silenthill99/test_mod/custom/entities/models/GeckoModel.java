package fr.silenthill99.test_mod.custom.entities.models;// Made with Blockbench 4.11.2
...
import fr.silenthill99.test_mod.custom.entities.animations.GeckoAnimations;
...
public class GeckoModel<T extends GeckoEntity> extends HierarchicalModel<T> {
	...
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(Main.MODID, "kaupengecko"), "main");
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.animateWalk(GeckoAnimations.MAGNAGECKO_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
		this.animate(entity.idleAnimationState, GeckoAnimations.MAGNAGECKO_IDLE, ageInTicks, 1f);
	}

	private void applyHeadRotation(float headYaw, float headPitch) {
		headYaw = Math.clamp(headYaw, -30f, 30f);
		headPitch = Math.clamp(headPitch, -25f, 25f);
